	...
	for {
		...
		for userID := startID + fac%2; userID < endID; userID += 2 {
			...
			}
			{
				...
				_, e := client.EnOrder(defaultCtx, &order)
				if e != nil {
					fmt.Println(e.Error())
					continue
				}
			...
			if count%1000 == 0 {
				...
				fmt.Printf("more 1000 enorder, total = %d, speed = %f\n", count, float64(count)/(float64(now-startTime)/float64(1*time.Second)))
			}
		}
	}
}
...
func main() {
	...
	flag.IntVar(&endID, "endid", 1000, "Testing end user ID")
